...
// in #list-3
  ...
  // $('#list-3 li:odd').css('marginLeft', '-20px'); // deprecated
  $('#list-3 li').filter( ':odd' ).css('marginLeft', '-20px');

  // for the first li set any different color
  $('#list-3 li:first').css('color', 'blue');

  // for all li with index > 5 set color to #ccc
  // $('#list-3 li:gt(5)').css('color', '#ccc'); // deprecated
  $('#list-3 li').slice(6).css('color', '#ccc');
...
$('img').wrap($('<div>'));
$('.mbox:last-of-type div:first-child').css({
  'float': 'left',
